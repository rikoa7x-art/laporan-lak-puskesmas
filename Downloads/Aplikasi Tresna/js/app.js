    ...
    setupSettings() {
        ...
        document.getElementById('sync-to-cloud')?.addEventListener('click', async () => {
            ...
            try {
                ...
                this.showToast('Data berhasil dicadangkan ke Cloud', 'success');
            } catch (error) {
                this.showToast(error.message || 'Gagal tersinkronisasi', 'error');
            } finally {
        ...
        document.getElementById('sync-from-cloud')?.addEventListener('click', async () => {
            ...
            if (!confirm('Ini akan mengunduh dan menimpa data lokal Anda dengan data di Cloud. Lanjutkan?')) {
                return;
            }
            ...
            try {
                ...
                this.updateProfileDisplay();
                this.updateDashboard();
                this.showToast('Data berhasil ditarik dari Cloud', 'success');
            } catch (error) {
                this.showToast(error.message || 'Gagal mengunduh data', 'error');
            } finally {
                btn.innerHTML = originalText;
                btn.disabled = false;
            }
